...
void matchDescriptors(cv::Mat &imgSource, cv::Mat &imgRef, cv::Mat &matchImg,
                      std::vector<cv::KeyPoint> &kPtsSource, std::vector<cv::KeyPoint> &kPtsRef, cv::Mat &descSource, cv::Mat &descRef,
                      std::vector<cv::DMatch> &matches, std::string descriptorType, std::string matcherType, std::string selectorType, 
                      std::vector<double> &t_matchDescriptors, bool bShowTimer)
{
    // Start the timer
    ...
    t_matchDescriptors.push_back(t);
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType, std::vector<double> &t_descKeypoints, bool bShowTimer)
{
    ...
    // Stop timer
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    t_descKeypoints.push_back(t);

    if (bShowTimer) cout << descriptorType << " descriptor extraction in " << 1000 * t / 1.0 << " ms" << endl;
}
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, std::vector<double> &t_detKeypoints, bool bShowTimer, bool bVis)
{
    // Start the timer
    double t = (double)cv::getTickCount();
